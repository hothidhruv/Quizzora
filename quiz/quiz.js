...
function submitData() {
    ...
    if (score == 0) {
        ...
        sendFaildEmail(score,quizdata,email);
    } else {
        debugger
        document.getElementById("quiz").style.display = "none";
        document.getElementById('score-container').style.display = 'inline';
        ...
        sendEmail(score,quizdata,email);
    }
...
function sendEmail(score,quizdata,getemail) {
    Email.send({
        SecureToken: "c6a9b381-6b83-4dda-822e-cd26f53f26de",
        To: getemail,
        From: "sbhayani850@gmail.com",
        Subject: "Exam Score(quizzora)",
        Body: `<table border="0" align="center" cellspacing="0" cellpadding="0" bgcolor="white" width="650">
  <tr>
    <td>
    <div style="background:url('https://i.ibb.co/jzpyV3w/124de3d1b5e12f1d8fcec1685e6343-unscreen.gif');">
      <!-- Child table -->
        ...
        <tr>
          <td>
            <h2 style="text-align:center; margin: 0px; padding-bottom: 25px; margin-top: 25px;">
              Quiz<span style="color:lightcoral">zora</span></h2>
          </td>
        </tr>
        <tr>
          <td>
            <img src="https://www.linkpicture.com/q/quizlogo.png" alt="Wisconsin Pollinators" height="50px" style="display:block;margin:auto;padding-bottom: 25px;">
          </td>
        </tr>
        <tr>
          <td style="text-align: center;">
            <h1 style="margin: 0px;padding-bottom: 25px; text-transform: uppercase;">Congratulations</h1>
            <h4 style="margin: 0px;padding-bottom: 25px;font-size:22px;">You have sucessfully submited quiz</h4>
            <h2 style="margin: 0px; padding-bottom: 25px;">Your Score is :</h2>
            <h2 style="bottom: 8%;margin: 0px;padding-bottom: 25px;position: absolute;">`+ score +`  Out of  `+ quizdata.length +`</h2>
          </td>
        </tr>
        <tr>
          <td style="text-align:center;">
            <h2 style="padding-top: 25px; line-height: 1; margin:0px;">Need Help?</h2>
            <div style="margin-bottom: 25px; font-size: 15px;margin-top:7px;">Contact us on sahilbhayni6745@gmail.com
            </div>
          </td>
        </tr>
      </table>
...
function sendFaildEmail(score,quizdata,getemail) {
...
//     var getquiztime = localStorage.getItem("quiztime");
//     firebase.database().ref('Quizdata').orderByChild("quizid").equalTo(quiztype).on('value', (snap) => {
//         snap.forEach(function (childSnapshot) {
//             var childKey = childSnapshot.key;

//             firebase.database().ref('Quizdata').child(childKey).update({
//                 'data': quiz,
//                 'quizid':quiztype,
